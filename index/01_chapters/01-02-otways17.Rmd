...
```{r otways17-map,   fig.align = "right", fig.cap = "Study area, western Otway Ranges, Victoria, Australia, showing the location of the camera-trapping sites (black dots) within the 3500 m buffer zone (thin grey line). The holes in the camera-trapping grids occurred due to rugged topography and limited access to these areas."}
...
stats <- c("Number of camera sites", "Sites where cats detected (%)", "Number of unmarked detection events", "Number of identifiable, marked detection events", "Number of unidentifiable, marked detection events", "Total number of identified individuals", "Number of cats resighted at different cameras", "Mean recapture distance (m)", "Maximum recapture distance (m)")
s_grid <- c(73, 51, 47, 60, 10, 23, 8, 653, 905)
n_grid <- c(67, 62, 48, 59, 5, 24, 6, 774, 1701)
b_grid <- c(140, 55, 95, 119, 15, 47, 14, 716, 1701)

df <- data.frame(stats, s_grid, n_grid, b_grid)

# make table with kableExtra
kbl(df, longtable = T, booktabs = T, digits = 2, 
    col.names = c("Summary statistic", "southern grid", "northern grid", "both grids"),
    caption = "Summary of raw camera survey data for feral cats in the Otway Ranges, Victoria, Australia, 2017.") %>% 
...
The lack of apex predators and competitors in the Otway Ranges may also facilitate high feral cat densities. Dingoes *Canis familiaris*&mdash;higher order predators [@johnson2007rarity]&mdash;and tiger quolls *Dasyurus maculatus*&mdash;key competitors [@glen2005complex]&mdash;are functionally extinct in the Otway Ranges. We detected foxes at 25% of sites (M. Rees, unpublished data) but the extent to which foxes exert top-down control on feral cats is unclear. Changes in feral cat abundance, behaviour and/or diet have been observed in response to fox control [@molsher2017mesopredator; @hunter2018not], and the relationship could be further clarified using robust density estimates under experimental manipulations of fox density.
