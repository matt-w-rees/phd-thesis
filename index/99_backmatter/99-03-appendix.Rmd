...
records_lg$grid  <- if_else(records_lg$st_no >= 65, "LGNP_n", "LGNP_s")
# add station_year
...
records$grid  <- if_else(grepl("^H", records$station), "Cobboboonee", records$grid)
...
#records$grid <- paste0(substr(records$station_year, 0, 1), substr(records$station_year, nchar(records$station_year) - 4, nchar(records$station_year)))

df <- records %>% 
  group_by(grid) %>% 
  mutate(year = substr(station_year, nchar(station_year) - 3, nchar(station_year)), 
         date_min = min(ymd(date_start)),
         date_max = max(ymd(date_end)),
         duration = (ymd(date_max) - ymd(date_min)) - 1)

# summarise fox naive occupancy
df <- df %>% 
  group_by(grid) %>% 
  mutate(fox_naive = round(sum(fox) / n(), digits = 2))
         
# consense to one row per grid
df <- df %>% 
  distinct(grid, .keep_all = TRUE) %>% 
  select(region, grid, year, date_min, date_max, duration, fox_naive) 

# rename grids / capitlise region
df$grid <- c("Annya", "Cobboboonee", "Hotspur", "Mt Clay", "North", "North", "North", "South", "South", "South", "LGNP South", "LGNP North")
...
df <- relocate(df, region, grid, replicate)

# make table
kbl(df, booktabs = TRUE, longtable = TRUE,
    col.names = c("Region", "Landscape", "Replicate", "Year", "Date start", "Date end", "Duration", "Naive fox occurrence"),
    caption = "Survey dates of camera-trapping grids and naive occurrence rates of foxes (proportion of camera-traps foxes were detected at) in each surveyed landscape") %>% 
...
```{r cam-models}
cam_models <- read.csv("data/density/cam_models.csv")
cam_models$X <- NULL

# make table
kbl(cam_models, booktabs = TRUE, longtable = TRUE,
    col.names = c("Region", "Camera model", "Frequency"),
    caption = "Frequency of different Reconyx camera-trap models used in each survey") %>% 
   kable_styling(latex_options = c("repeat_header"), font_size = 10) 
```
